...
##1 ---------x minBase=94.08 maxBase=94.08 cnt=

regex= re.compile(r'##1 ---------x\s+minBase=(\S+)\s+maxBase=(\S+)\s+cnt=(\d+)')
line =             '##1 ---------x   minBase=94.08   maxBase=94.08   cnt=3'
m = regex.search(line)
assert m
	...
	with open(path, 'rt', errors='ignore') as f:
		for line in f:
			...
			if not m:
				continue
			n += 1
			minBase = float(m.group(1))
			maxBase = float(m.group(2))
			cnt = float(m.group(3))
